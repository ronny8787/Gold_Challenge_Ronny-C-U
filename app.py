...
if selected == "Input Text":
    ...
    with col1:
        ...
        kalimat_data = st.text_area ("Input text :",kalimat_data, height = 200,label_visibility="collapsed")
        st.write (f"Data Type of Input Text :  {type(kalimat_data)}")
        ...
        if running_cleansing_text:
            ...
            if all_option_check :
                if stemming_check or lowercase_check or remove_number_check or remove_punctuation_check or tokenization_check or normalization_check or stopwords_check == True :
                    ...
                    kalimat_data = remove_emoji (kalimat_data)
                    kalimat_data = lowercase (kalimat_data)
                    kalimat_data = threeormore (kalimat_data)
                    kalimat_data = stemming (kalimat_data)
                    kalimat_data = tokenization (kalimat_data)
                    kalimat_data = normalization (kalimat_data)
                    kalimat_data = remove_number (kalimat_data)
                    kalimat_data = remove_punctuation (kalimat_data)
                    kalimat_data = stopwords (kalimat_data)
                    kalimat_data = to_string (kalimat_data)
    ...
    with col2: 
        ...
        st.text_area ("Output text :", value = kalimat_data, height = 200,label_visibility="collapsed")
        st.write (f"Data Type of Output Text :  {type(kalimat_data)}")
...
if selected == "Input Text from CSV":
    ...
    with col3: 
        ...
        if running_cleansing_text_2:
            ...
            # Tokenization
            if tokenization_check :
                text_data = tokenization (kalimat_data)
...
if selected == "Input Multiple Text from CSV":
    ...
    with col2: 
        ...
        kalimat_data = stopwords (kalimat_data)
        kalimat_data = to_string (kalimat_data)
        words = pd.Series(kalimat_data.split())
    ...
    with col3:
        ...
        wordcloud = WordCloud(background_color = 'white').generate(kalimat_data)
